...
def generate_launch_description():
    ...
    # Image generation
    img_gen_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [
    ...
    ld.add_action(img_gen_launch)

    # Cloud generation
    cloud_gen_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [
                    FindPackageShare("sick_images"),
                    "launch",
                    "point_cloud_generator.launch.py",
                ]
            )
        )
    )
    ld.add_action(cloud_gen_launch)

    # Processing
    proc_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [
                    FindPackageShare("sick_images"),
